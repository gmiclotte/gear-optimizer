        ...
        return (<label key={this.props.factors[this.props.idx]}>
            ...
            <select value={this.state.value} onChange={this.handleChange}>
                {Object.getOwnPropertyNames(Factors).map((factor) => (
                    <option value={Factors[factor][0]} key={Factors[factor][0]}>{Factors[factor][0]}</option>))}
            </select>
            <Crement header='slots' value={this.props.maxslots[this.props.idx]} name={['maxslots', this.props.idx]}
                     handleClick={this.props.handleCrement} min={0} max={accslots}/>
        </label>);
    }
}
