...
['My Blue Heart <3', 'SetName.MISC'],
['My Brown Heart <3', 'SetName.MISC'],
['My Green Heart <3', 'SetName.MISC'],
['My Red Heart <3', 'SetName.MISC'],
['My Yellow Heart <3', 'SetName.MISC'],
['My Orange Heart <3', 'SetName.MISC'],
['My Purple Heart <3', 'SetName.MISC'],
['My Grey Heart <3', 'SetName.MISC'],
['My Pink Heart <3', 'SetName.MISC'],
...
def suffix():
    ...
    print('')
    print('')
    print('export const LOOTIES = ITEMLIST.filter(x => x.zone !== undefined && x.zone[0] === SetName.LOOTY[0]).sort((a, b) => a.power - b.power).map(x=>x.name);')
    print('export const PENDANTS = ITEMLIST.filter(x => x.zone !== undefined && x.zone[0] === SetName.FOREST_PENDANT[0]).sort((a, b) => a.power - b.power).map(x=>x.name);')
...
with open('ItemNameDesc.cs', 'r') as file:
    ...
    line = 0
    while line < len(text):
        if text[line][:13] == 'this.itemName' or text[line] == '}':
            if tmp[0][:13] == 'this.itemName':
                items += [tmp]
            tmp = []
        tmp += [text[line]]
        line += 1
...
for idx in range(len(items)):
    ...
    for jdx in range(len(item)):
        ...
        pos1 = entry.index('[')
        pos2 = entry.index('=')
        props += [[entry[5:pos1], entry[pos2 + 1:-1].replace('"', '').strip()]]
    props2item(props)
suffix()
